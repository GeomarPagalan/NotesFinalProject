<?php
	...
	if($_POST['action']=='add_note'){
		...
		echo $prepared_statement->execute(array($name, $description, 1, 1));
	...
	}else if($_POST['action']=='get_notes')
	{
		...
		echo $prepared_statement->execute(array($note_id));
			$pdo->commit();
			echo "deleted";
		}catch(Exception $e){
			$pdo->rollback();
		...
		echo $prepared_statement->execute(array($_POST['data']['name'],$_POST['data']['description'],date("Y-m-d H:i:s"),$_POST['data']['id']));
			$pdo->commit();
			echo "edited";
		}catch(Exception $e){
			$pdo->rollback();
